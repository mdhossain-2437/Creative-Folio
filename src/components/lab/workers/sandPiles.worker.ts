...
const sandInit: WorkerInitFn = ({ w, h, store, compact }) => {
  const cell = compact ? 6 : 4;
  const cols = Math.floor(w / cell);
  const rows = Math.floor(h / cell);
  store.grid = new Uint8Array(cols * rows);
  store.cols = cols;
  store.rows = rows;
  store.cell = cell;
};

const sandTick: WorkerTickFn = ({ ctx, w, h, m, store }) => {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (!value) continue;
      ctx.fillStyle = value === 2 ? "#6a655f" : "#e3bfb4";
