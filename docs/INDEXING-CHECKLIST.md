...
> every page indexed and to rank for the branded query *Delowar Hossain* on
> Google, Bing, and Yahoo, with the portrait + Knowledge Panel on the right.
>
...
2. **Bing Webmaster Tools** → *Sitemaps* → submit
   `https://delowarhossain.dev/sitemap.xml`.
...
```
https://delowarhossain.dev/
https://delowarhossain.dev/about
https://delowarhossain.dev/ai
https://delowarhossain.dev/resume
https://delowarhossain.dev/works
https://delowarhossain.dev/contact
```
...
The key file is live at `https://delowarhossain.dev/indexnow.txt`. After a
deploy, POST the changed URLs to the built-in endpoint:

```bash
curl -X POST https://delowarhossain.dev/api/indexnow \
  -H "Content-Type: application/json" \
  -d '{"urls":["https://delowarhossain.dev/","https://delowarhossain.dev/about"]}'
```
...
`delowarhossain.dev` in its website field, and the site's `Person.sameAs`
(in `src/lib/site.ts > socials`) must list that profile back.

- [ ] GitHub bio → website = `delowarhossain.dev`
- [ ] LinkedIn → Contact info → website = `delowarhossain.dev`
